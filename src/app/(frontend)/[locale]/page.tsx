...
export default async function HomePage(props: { params: Promise<{ locale: Locale }> }) {
  ...
  // Fallback hero shown until the "home" page is created/migrated in the admin.
  const t = await getTranslations('home')
      ...
      <section className="relative min-h-[85vh] flex items-center overflow-hidden bg-black text-white py-20">
        ...
        <div className="relative mx-auto max-w-7xl px-4 w-full z-10">
          <div className="max-w-4xl">
            <span className="inline-flex rounded border border-brand-yellow bg-black/40 px-3 py-1 text-[10px] font-black uppercase tracking-[0.25em] text-brand-yellow">
            ...
            <div className="mt-14 max-w-2xl relative p-6 md:p-8 border-l-4 border-r-4 border-brand-yellow/80 bg-black/55 backdrop-blur-xs overflow-hidden rounded">
              ...
              <p className="mt-3 text-xs font-semibold uppercase tracking-widest text-brand-yellow/90">{statSource}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-20 text-black border-t border-black">
        <div className="mx-auto max-w-7xl px-4">
          <div className="max-w-3xl mb-12">
            <h2 className="text-3xl font-black leading-tight tracking-tight uppercase md:text-5xl">{sectionTitle}</h2>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {focusAreas.map((item, idx) => (
              ...
              >
                ...
                <div className="absolute top-4 right-4 text-xs font-bold text-black/25">0{idx + 1}</div>
        ...
        <div className="mx-auto max-w-4xl px-4 text-center">
          <div className="relative inline-block py-12 px-8 border-t border-b border-brand-yellow/40">
            ...
            <p className="mt-6 text-xs font-bold uppercase tracking-[0.25em] text-brand-yellow">
              {isThai ? '— แรงงานประมงสัญชาติเมียนมา ที่ได้รับความช่วยเหลือในน่านน้ำสากล' : '— Rescued Migrant Fisher, Assisted in International Waters'}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-20 text-black border-b border-black">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 max-w-5xl mx-auto">
            {[
              { lang: 'ภาษาไทย (Thai)', code: 'TH', phone: '084-434-6663', label: 'Call Thai Line' },
              { lang: 'မြန်မာဘာသာ (Burmese)', code: 'MM', phone: '084-434-6664', label: 'Call Burmese Line' },
              { lang: 'ភាសាខ្មែរ (Khmer)', code: 'KH', phone: '084-434-6665', label: 'Call Khmer Line' },
              { lang: 'ພາສາລາວ (Lao)', code: 'LA', phone: '084-434-6666', label: 'Call Lao Line' },
            ].map((hotline) => (
              <div key={hotline.code} className="border border-black rounded p-6 flex flex-col items-center text-center bg-white shadow-xs">
